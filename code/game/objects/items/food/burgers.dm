...
/obj/item/food/burger/ghost/process()
	...
	switch(paranormal_activity)
		if(97 to 100)
			...
			playsound(src, 'sound/misc/chain_rattling.ogg', 300, TRUE)
		if(91 to 96)
		...
		if(65 to 70)
			...
			light?.flicker()
		if(62 to 64)
			playsound(src, pick('sound/hallucinations/i_see_you1.ogg', 'sound/hallucinations/i_see_you2.ogg'), 50, TRUE, ignore_walls = FALSE)
		if(61)
			visible_message("[src] spews out a glob of ectoplasm!")
			new /obj/effect/decal/cleanable/greenglow/ecto(loc)
			playsound(src, 'sound/effects/splat.ogg', 200, TRUE)
