...
/obj/machinery/ammo_workbench/proc/fill_round(casing_type)
	...
	if(new_casing.type in possible_ammo_types)
		...
		use_energy(3 KW)
		playsound(src, 'sound/machines/piston_raise.ogg', 60, 1)
	else
		qdel(new_casing)
		ammo_fill_finish(FALSE)
		return
...
/obj/machinery/ammo_workbench/proc/Insert_Item(obj/item/O, mob/living/user)
	...
	if(istype(O, /obj/item/ammo_box))
		...
		update_appearance()
		update_ammotypes()
		playsound(src, 'sound/weapons/autoguninsert.ogg', 35, 1)
		return TRUE
	if(istype(O, /obj/item/disk/ammo_workbench))
		...
		flick("h_lathe_load", src)
		update_appearance()
		playsound(src, 'sound/machines/terminal_insert_disc.ogg', 35, 1)
		return TRUE
	return FALSE
