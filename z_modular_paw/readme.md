# The modularization handbook - Paw Sector style, v0.2
...
```diff
- var/something = 1
+ var/something = 2 // PAW EDIT CHANGE - ORIGINAL: var/something = 1
```
...
```byond
var/something = 2 // PAW EDIT CHANGE - ORIGINAL: var/something = 4
```
...
In general they are a good idea and encouraged whenever it is possible to do so. However this is not a hard rule, and sometimes Paw edits are preferable. Just try to use your common sense about it.
...
Performance should also be considered when you are overriding a hot proc (like Life() for example), as each additional call adds overhead. Paw edits are much more performant in those cases. For most procs this won't be something you have to think about, though.
...
- **Addition:**
  ...
  ```byond
  // PAW EDIT ADDITION START - SHUTTLE_TOGGLE - (Optional Reason/comment)
...
- **Removal:**
  ...
  /* // PAW EDIT REMOVAL START - SHUTTLE_TOGGLE - (Optional Reason/comment)
  ...
  *Please avoid this however, only to be done if there is no other option.
  ...
  ```byond
  /* // PAW EDIT REMOVAL START - SHUTTLE_TOGGLE - (Moved to modular_nova/shuttle_toggle/randomverbs.dm)
...
- **Change:**
  ...
  ```byond
  if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE, SHUTTLE_DISABLED) // PAW EDIT CHANGE - ORIGINAL: if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE)
  ```
  ...
  ```byond
  /* // PAW EDIT REMOVAL START - Adds conditional
  	return 1
  */ // PAW EDIT REMOVAL
  // PAW EDIT ADDITION START - Adds conditional
  	if(!isnull(src))
  		return 1
  // PAW EDIT ADDITION END
...
ALL of the tgui files are located in `/tgui/packages/tgui/interfaces` and its subdirectories; there is no specific folder for Paw Sector UIs.
...
You can do both `// PAW EDIT` and `/* PAW EDIT */`, though in some cases you may have to use one over the other.
...
<Button
	onClick={() => act('spin', { high_quality: true })}
	// PAW EDIT ADDITION START - another example, multiline changes
	icon="rat"
...
```js
// THIS IS A PAW SECTOR UI FILE
```

This way they are easily identifiable as modular TGUI .tsx/.jsx files. You do not have to do anything further, and there will never be any need for a Paw edit comment in a modular TGUI file.
