...
This guide will help you in pulling Pull requests from TG and adding them into a PR to be merged here in Paw. Thank you to Nova sector for writing this guide. Adjustments have been made neccessary for Monkestation

## _DO NOTE ALL OF THIS SHOULD BE DONE ON A FORK OF PAW'S REPO, IF YOU DIRECTLY CLONED THE REPO YOU WILL NOT BE ABLE TO DO THIS_
...
Once in there, add monkestation as a remote if you do not already have that done on your fork. Simply run the following command:

`git remote add monkestation https://github.com/monkestation/monkestation`

This will ad TG's repo as a remote under the name of monkestation. **You only need to do this step once.**
...
Using this Pull Request as an [Example](https://github.com/monkestation/monkestation/pull/12597)
...
https://github.com/Monkestation/Monkestation2.0/commit/73925faf43abe24ed8f2fceb7f41f53a071ca6d0

1. Simply click on the abbreviated hash link (the `73925f` shown in the example above), and in the url address bar copy full version of the hash just after /commit/
   In this example it should be `73925faf43abe24ed8f2fceb7f41f53a071ca6d0`
...
2. To grab this singular commit, assuming the previous steps have been completed you would just need to run the following command:
   `git cherry-pick -m 1 73925faf43abe24ed8f2fceb7f41f53a071ca6d0`
...
In this example the hash for the changelog is this. `dc0345c62c9ca5d14c54cb174de323e1e5c5e959`
...
https://github.com/Monkestation/Monkestation2.0/pull/12597/commits/679fe85e462211f913cbeb5b911b34d0ee17df59
`git cherry-pick -m 1 679fe85e462211f913cbeb5b911b34d0ee17df59^..dc0345c62c9ca5d14c54cb174de323e1e5c5e959`
...
Now you are ready to open your [pull request](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request)! You can either visit github and open your fork on the website and go through the process of making a Pull Request, or use the github desktop app which has a button to take you directly to making a Pull request. Fork also has this option in the left hand pane if you right click Monkes-Paw master branch.
...
This next portion of the guide is basically the above but using Git-Fork which helps the process of pulling multiple PR's into a new Pull Request for Monkes-Paw.
...
From there, name the remote as you want I.E. 'monkestation' and for the repository URL put `https://github.com/monkestation/monkestation`
...
Now to to make the PR pulling changes from Monkestation.
...
From here it is super simple to add in the the Pull requests from Monkestation you wish to add to your pull request to merge into Paw.
On the side panel where it lists remotes, click on the filter button with the tooltip `show branches from here only`
**It is recommended to pin the monkestation master branch to the left panel so you can easily click the filter button (to the right of the pin button) when you need to. Clear branch filter is Ctrl+Shift+A, use this to quickly swap between tg's commits and your own branches**
...
And then from the main view, simply hold Ctrl and click on the PR + its changelog for each and every Pull Request you wish to copy over to Paw. Right click on one and then click on 'Cherry-pick...' and then click 'Cherry Pick `n` commits' on the resulting dialog.
...
Then from there, you can simply push changes and either fix any conflicts or errors, and then use Fork, Github Desktop, or go to github to make your Pull Request to Paw.
...
(Note: the "Rebase and Merge" option may also be used, at your own preference. Both will keep the commit history intact but have their own advantages/disadvantages. We exclusively use Merge commits here on Paw).
